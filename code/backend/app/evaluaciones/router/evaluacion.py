...
from app.evaluaciones.schema.evaluacion import RegistrarEvaluacion,MostrarEvaluacion,MostrarUltimaEvaluacion
...
def mostrar_ultima_evaluacion_de_paciente(paciente_id:int,db:Session=Depends(get_db)):
    ...
    ultima_evaluacion_del_paciente_obtenida=db.query(Evaluacion).filter(Evaluacion.paciente_id==paciente_id).order_by(Evaluacion.nro_evaluacion.desc()).first()
